...
bool Vcnl4030::isConnected() {
	return I2cDriver::isConnected(VCNL_ADD);
}
...
bool Vcnl4030::initialize(void){
	...
	if (!I2cDriver::write(VCNL_ADD, als_config, sizeof(als_config)))
		return false; // 通信失敗
	...
	if (!I2cDriver::write(VCNL_ADD, ps_config, sizeof(ps_config)))
		return false; // 通信失敗
...
bool Vcnl4030::readALS(float * als)
{
	...
	if (!I2cDriver::write(VCNL_ADD, enable_als, sizeof(enable_als)))
		return false; // 通信失敗
	...
	uint8_t buffer[2];
	if (!I2cDriver::writeRead(VCNL_ADD, &read_command, 1, buffer, sizeof(buffer))) 
		return false; // 通信失敗
	...
	if (!I2cDriver::write(VCNL_ADD, disable_als, sizeof(disable_als)))
		return false; // 通信失敗
...
bool Vcnl4030::readPS(float * ps)
{
	*ps = 0; // 初期化
	...
	if (!I2cDriver::write(VCNL_ADD, force_mode, sizeof(force_mode)))
		return false; // 通信失敗
	...
	if (!I2cDriver::write(VCNL_ADD, enable_ps, sizeof(enable_ps)))
		return false; // 通信失敗
	...
	if (!I2cDriver::write(VCNL_ADD, disable_ps, sizeof(disable_ps))) 
		return false; // 通信失敗
