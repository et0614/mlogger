...
!!! note "v3 / v4 で取得方法が異なります"
    - **v4** は内蔵フラッシュなので、計測後に **スマートフォン (Bluetooth)** または **PC (USB Type-C)** で読み出します (Bluetooth 経由の操作は [記録データの管理](#記録データの管理) 参照)。
    - **v3** は本体からメモリカードを物理的に取り出して、PC のカードリーダで読み出します。スマートフォンや USB ケーブル経由での取り出しには対応していません。
...
!!! note "v4 ファームウェアのみ表示"
    本セクションは v4 ファームウェアでのみ画面に出てきます。v3 ファームウェアでは本体からメモリカードを物理的に取り出して PC のカードリーダで読み出します。
